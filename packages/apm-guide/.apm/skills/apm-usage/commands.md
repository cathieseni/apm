...
| `apm marketplace plugin add <source>` | Add a plugin entry to `marketplace.yml` | `--name`, `--version`, `--ref`, `--description`, `--subdir`, `--tag-pattern`, `--tags`, `--include-prerelease`, `--no-verify` |
| `apm marketplace plugin set <name>` | Update fields on an existing plugin entry | `--version`, `--ref`, `--description`, `--subdir`, `--tag-pattern`, `--tags`, `--include-prerelease`, `--no-verify` |
