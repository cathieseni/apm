...
APM uses APM to manage its own agent primitives. The `.apm/` tree is the source of truth; `AGENTS.md`, `CLAUDE.md`, and `.github/copilot-instructions.md` are generated by `apm compile` and CI enforces sync via `apm compile --check`. See [CONTRIBUTING.md -- Recompiling agent outputs](CONTRIBUTING.md#recompiling-agent-outputs) for the workflow.
